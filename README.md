...
- [Production vs Local Version](#production-vs-local-version)
...
## Production vs Local Version

The original pipeline was built and executed inside a client's secure VDI environment with access to live hospital EMR systems, restricted private networks, and production-grade security controls. This repository is a local version built for learning and portfolio purposes — every architecture pattern, pipeline structure, and notebook logic is faithfully reproduced, but against development Azure resources rather than production ones.
...
| Secrets management | Azure Key Vault (no credentials in code) | Placeholder config values (`STORAGE_ACCOUNT`, connection strings) |
| Pipeline schedule | Automated runs on business schedule | Manual trigger only |
| Unity Catalog | Production catalog with full governance policies | Dev catalog `rcm_adb` |
| Data access controls | Private endpoints, VNet integration, Azure AD authentication | Open dev environment config |
